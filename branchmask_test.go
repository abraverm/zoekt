...
func TestCopyMask(t *testing.T) {
	...
	if len(c) != len(original) {
		t.Fatalf("copy length = %d, want %d", len(c), len(original))
	}
	for i := range original {
		if c[i] != original[i] {
			t.Errorf("copy[%d] = %02x, want %02x", i, c[i], original[i])
		}
	}
...
func TestCopyMaskEmpty(t *testing.T) {
	...
	c := copyMask(original)
	if len(c) != 0 {
		t.Errorf("copy of empty mask should be empty, got length %d", len(c))
	}
}
